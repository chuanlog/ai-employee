...
- 工单处理完成后，系统会异步将问题、处理结果和回访记录沉淀为 FAQ 文档，并复用知识库链路写入 MinIO、MySQL 和 Milvus 向量索引。
...
| 工具/方式                   | 用途               | 说明                                                                |
| ----------------------- | ---------------- | ----------------------------------------------------------------- |
| IntelliJ IDEA           | 后端开发与调试          | 用于 Spring Boot 后端代码编写、断点调试、Maven 依赖管理和测试运行。                       |
| Trae                    | 前端开发/ vibecoding | 基于 Trae 进行需求拆解、代码生成、重构建议、问题定位和文档补充、单测生成。本项目单元测试几乎完全由trae 根据后端代码生成 |
| Docker / Docker Compose | 本地依赖环境配置/云端环境部署  | 开发依赖的组件通过docker compose文件在云端快速搭建，一次搭建，大家共用，不用在本地虚拟机上折腾.           |
| Maven                   | 后端构建与测试          | 负责依赖解析、项目编译、单元测试执行和 Spring Boot 应用启动。                             |
| npm / Vite              | 前端开发与构建          | 负责前端依赖安装、开发服务器启动、生产构建和本地预览。                                       |
...
处理工单，支持填写处理结果和回访信息。处理完成后会异步生成 `ticket-{id}-faq.md` 知识文档，并完成向量化入库，供后续 RAG 检索复用。
